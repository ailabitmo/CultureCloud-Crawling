...
def get_image_url(object_id)
  "http://culturecloud.ru/image/#{object_id}.jpg"
end
...
get_artwork_ids.each { |artwork_id|
  ...
  title_uri = RDF::URI.new("#{@rdf_prefixes[:cc].to_s}object/#{artwork_id}/title/1")
  @graph_artwork << [title_uri, RDF.type, @ecrm[:E35_Title]]
  @graph_artwork << [title_uri, RDF.type, OWL.NamedIndividual]
  @graph_artwork << [artwork_uri, @ecrm[:P102_has_title], title_uri]
  titles.each do |loc, title|
    @graph_artwork << [artwork_uri, RDFS.label, RDF::Literal.new(title, :language => loc)]
    @graph_artwork << [title_uri, RDFS.label, RDF::Literal.new(title, :language => loc)]
  end
  ...
  @graph_artwork << [artwork_uri, @ecrm[:P138i_has_representation], artwork_image_uri]
