  ...
  q = RDF::Query.new({
                       :obj => {
                           RDF::URI.new('http://erlangen-crm.org/current/P14_carried_out_by') =>
                               RDF::URI.new("http://culturecloud.ru/resource/person/#{person_id}")
                       }})
  production = q.execute(@artwork_ownerships).first[:obj].to_s
  object_id = /http:\/\/culturecloud.ru\/resource\/object\/([0-9]+)\/production\/?/.match(production)[1]
  crawl_bio(object_id)
end
...
def gen_appellation_uri(person_id)
  RDF::URI.new("http://culturecloud.ru/resource/person/#{person_id}/appellation/1")
end
